...
```
— выводит обезличенную информацию о вебхуке (хост, team/channel, длина токена). Полезно для проверки корректности `SLACK_URL`.
...
## Частые проблемы
* **Slack возвращает 404.** Вебхук удалён или ссылка указана с ошибкой. Сгенерируйте новый URL, обновите `.env` и перезапустите скрипт.
